...
$z_get = 'keyword';//название GET переменной для go.php (http://doorway.com/go.php?q=keyword)
...
if($z_conf_edit == 0 || ($z_conf_edit == 1 && empty($z_test_config))){
	...
	$z_conf['rd_bots'] = 1;//запрашивать с TDS данные для ботов (0/1)
...
$z_out = $z_macros = $z_cookies_data = '';
$z_empty = $z_bot = $z_useragent = $z_referer = $z_se = $z_cf_country = $z_lang = $z_country = $z_city = $z_region = $z_asn = $z_org = $z_device = $z_operator = $z_os_name = $z_os_version = $z_br_name = $z_br_version = $z_brand = $z_page = '-';
$z_uniq = 'yes';
...
if($z_conf['status'] == 1){
	...
	if(!empty($_SERVER['HTTP_REFERER'])){
		...
		elseif(strstr($z_referer, 'yandex.') || strstr($z_referer, '/ya.ru')){$z_se = 'yandex';}
		elseif(strstr($z_referer, 'mail.ru')){$z_se = 'mail';}
		elseif(strstr($z_referer, 'yahoo.')){$z_se = 'yahoo';}
		elseif(strstr($z_referer, 'bing.com')){$z_se = 'bing';}
		elseif(strstr($z_referer, 'baidu.com')){$z_se = 'baidu';}
		elseif(strstr($z_referer, 'duckduckgo.')){$z_se = 'duckduckgo';}
	}
	...
	if(
		stristr($z_useragent, 'googlebot') ||
		stristr($z_useragent, 'mediapartners-google') ||
		stristr($z_useragent, 'adsbot-google') ||
		stristr($z_useragent, 'apis-google') ||
		stristr($z_useragent, 'google-read-aloud') ||
		stristr($z_useragent, 'feedfetcher-google') ||
		stristr($z_useragent, 'googleimageproxy') ||
		stristr($z_useragent, 'google-safebrowsing')
	){
		$z_bot = 'google';
	}
	elseif(
		stristr($z_useragent, 'bingbot') ||
		stristr($z_useragent, 'msnbot') ||
		stristr($z_useragent, 'adidxbot') ||
		stristr($z_useragent, 'outlook') ||
		stristr($z_useragent, 'microsoft office') ||
		stristr($z_useragent, 'bf-directline')
	){
		$z_bot = 'bing';
	}
	elseif(
		stristr($z_useragent, 'yandex.com/bots') ||
		stristr($z_useragent, 'yandexbot') ||
		stristr($z_useragent, 'yandexmailproxy')
	){
		$z_bot = 'yandex';
	}
	elseif(
		stristr($z_useragent, 'yahoo.com') ||
		stristr($z_useragent, 'yahoo! slurp') ||
		stristr($z_useragent, 'yahoomailproxy')
	){
		$z_bot = 'yahoo';
	}
	elseif(stristr($z_useragent, 'duckduckbot')){
		$z_bot = 'duckduckgo';
	}
	elseif(stristr($z_useragent, 'baidu.com')){
		$z_bot = 'baidu';
	}
	elseif(stristr($z_useragent, 'mail.ru')){
		$z_bot = 'mail';
	}
	elseif(
		stristr($z_useragent, 'facebook') ||
		stristr($z_useragent, 'facebot')
	){
		$z_bot = 'facebook';
	}
